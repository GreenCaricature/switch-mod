var parseURI = function parse_uri(str) {
    if (str === "@dev" || str === "@qa" || str === "@prod") return {
        path: str
    };
    var o = {
        strictMode: false,
...
var RuleMatcher = function(rules) {
    ...
    var cachebust = function(url) {
        if (has(url, /\?/gim)) return url + "&_cb=" + Math.random().toFixed(16).substring(2, 18);
        else return url + "?_cb=" + Math.random().toFixed(16).substring(2, 18)
    };
    var lastRequestId;
    this.rules = rules;
